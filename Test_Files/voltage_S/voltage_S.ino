/*
voltage reading
*/

float vol_read;
float vol_Sample = 0.0;
float x,V;

void setup() {
  pinMode( A1, INPUT);
  Serial.begin(9600);
}

void loop() {
//  vol_Sample = 0.0;
//  vol_read = analogRead(A1);
//  Serial.println(vol_read);
//  delay(100);

//--------------------------------------------------------

//  for ( int i=0; i< 100; i=i+1) {
//    vol_read = analogRead(A1)-512;
//    vol_Sample = vol_Sample + sq(vol_read);
//  
//  }
//
//  float vol_mean = vol_Sample/100.0;
//  float RMSvol_mean = (sqrt(vol_mean))*1.5;
//
//  Serial.print("voltage :");
//  Serial.println(RMSvol_mean);


//---------------------------------------------------------


//  float V2 =0;
//  float Vsquare =0;
//  for ( int i=0; i<100; i=i+1) {
//    x = analogRead(A1)-512;
//    V = 0.00000412*x*x*x-0.000857*x*x+2.675*x-3.198;
//    V2 = V*V;
//    Vsquare = Vsquare + V2;
//    delay(0.1);
//  }
//
//  float Vrms = sqrt( Vsquare/100);
//  Serial.println(Vrms);
//  delay(500);

// ------------------------------------------------------------
  RMSvol_mean = 0.0;
    
  VsensorRead = getVVPP();
  VReadRMS = (VsensorRead/2.0) *0.707;   //root 2 is 0.707
  RMSvol_mean = (VReadRMS * 1000)/mVperV;

 
}

float getVVPP()
{
  float Vresult;
  int VreadValue;                // value read from the sensor
  int VmaxValue = 0;             // store max value here
  int VminValue = 1024;          // store min value here
  
   uint32_t start_time = millis();
   while((millis()-start_time) < 1000) //sample for 1 Sec
   {
       VreadValue = analogRead(A0);
       // see if you have a new maxValue
       if (VreadValue > VmaxValue) 
       {
           /*record the maximum sensor value*/
           VmaxValue = VreadValue;
       }
       if (VreadValue < VminValue) 
       {
           /*record the minimum sensor value*/
           VminValue = VreadValue;
       }
   }
   
   // Subtract min from max
   Vresult = ((VmaxValue - VminValue) * 5.0)/1024.0;
      
   return Vresult;
 }
